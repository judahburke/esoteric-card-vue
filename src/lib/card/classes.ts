import { cardErrorKeys } from './constants'
import { defaultCutOptions, defaultDealOptions, defaultShuffleOptions } from './constants';
import type { ICardRank, ICardSuit, ICard, ICardFactory, IDealer, IDeck, ITable, IPlayer } from './interfaces';
import type { CutOptions, ShuffleOptions, DealOptions, CardRankKey, CardSuitKey, CardSuitColorKey } from './types';

class StandardCard implements ICard<StandardCardRank, StandardCardSuit, StandardCard> {
    ...
    compare(other: StandardCard): number {
        let r = this.rank.compare(other.rank);
        let s = this.suit.compare(other.suit);
        return s === 0 ? r : s;
    }
}
...
abstract class GenericDeck<Rank extends ICardRank<Rank>, Suit extends ICardSuit<Suit>, Card extends ICard<Rank,Suit,Card>>
    ...
    cut(topLength?: number, options?: CutOptions): void {
        if (!options) { options = defaultCutOptions; }
        if (options.cutMinimum > this._stack.length) {
            throw new Error(cardErrorKeys.TABLE_400_CUT);
        }
        if (!topLength) {
            const min = Math.min(options.cutMinimum, this._stack.length - options.cutMinimum);
            const max = Math.max(options.cutMinimum, this._stack.length - options.cutMinimum);
            topLength = Math.floor((Math.random() * (max - min)) + min);
        }
        if (topLength < options.cutMinimum || topLength > this._stack.length - options.cutMinimum) {
            throw new Error(cardErrorKeys.TABLE_400_CUT);
        }

        let top = this._stack.slice(0, topLength);
        let bot = this._stack.slice(topLength);
        this._stack = bot.concat(top);
    }
    shuffle(dealtCards?: Card[], options?: ShuffleOptions): void {
        if (!dealtCards) { dealtCards = []; }
        if (!options) { options = defaultShuffleOptions; }
        let allCards = this._stack.concat(dealtCards);
        for (let iteration = 0; iteration < options.shuffleCount; iteration++) {
            let left = allCards.slice(0, allCards.length / 2);
            let right = allCards.slice(allCards.length / 2);
            allCards = [];
            while (left.length > 0 || right.length > 0) {
                let card = left.length > 0 && Math.random() < 0.5
                    ? left.pop()
                    : right.pop();
                if (card) {
                    allCards.push(card);
...
class StandardDeck extends GenericDeck<StandardCardRank, StandardCardSuit, StandardCard> {}

abstract class GenericDealer<Rank extends ICardRank<Rank>, Suit extends ICardSuit<Suit>, Card extends ICard<Rank,Suit,Card>>
        implements IDealer<Rank, Suit, Card> {
    deal(deck: IDeck<Rank, Suit, Card>, options?: DealOptions): Card[] {
        if (!options) { options = defaultDealOptions; }
        const cards: Card[] = [];
...
class StandardDealer extends GenericDealer<StandardCardRank, StandardCardSuit, StandardCard> {}
...
abstract class GenericTable<Player extends IPlayer<Player>> implements ITable<Player> {
    public get length(): number {
    ...
    public setNext(): Player {
        console.debug('[card] GenericTable.setNext (pre)', this._currentDealer, this._currentLeader);
        const player = this.peekNextLeader();
        this._currentDealer = this.fix(this._currentDealer + 1);
        console.debug('[card] GenericTable.setNext (post)', this._currentDealer, this._currentLeader);
        return player;
    }
    public setNextDealer(player: Player): void {
        console.debug('[card] GenericTable.setNextDealer (pre)', this._currentDealer, this._currentLeader);
        const i = this._seats.findIndex(p => p.compare(player) === 0);
        if (i < 0) {
            throw new Error(cardErrorKeys.TABLE_404_NEXT);
        }
        this._currentDealer = i;
        console.debug('[card] GenericTable.setNextDealer (post)', this._currentDealer, this._currentLeader);
    }
    public setNextLeader(player: Player): void {
        console.debug('[card] GenericTable.setNextLeader (pre)', this._currentDealer, this._currentLeader);
        const i = this._seats.findIndex(p => p.compare(player) === 0);
        if (i < 0) {
            throw new Error(cardErrorKeys.TABLE_404_NEXT);
        }
        this._currentDealer = this.fix(i - 1);
        console.debug('[card] GenericTable.setNextLeader (post)', this._currentDealer, this._currentLeader);
    }
