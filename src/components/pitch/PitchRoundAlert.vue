...
const bid = computed(() => {
  if (props.calculation.greatestBid) {
    return {
      team: props.calculation.greatestBid.team.name,
...
const high = computed(() => {
  if (props.calculation.highestTrump) {
    return {
      team: props.calculation.highestTrump.team.name,
...
const low = computed(() => {
  if (props.calculation.lowestTrump) {
    return {
      team: props.calculation.lowestTrump.team.name,
...
const jack = computed(() => {
  if (props.calculation.jackiestTrump) {
    return {
      team: props.calculation.jackiestTrump.team.name,
...
const game = computed(() => {
  if (props.calculation.greatestGame) {
    let teams = "";
    props.calculation.greatestGame.teams.forEach(
      (t) => (teams += t.name + "|")
    );
    return {
...
const teamsWithScore = computed(() => {
  ...
  if (
    props.calculation.greatestBid &&
    !teams.some((t) => t.compare(props.calculation.greatestBid!.team) === 0)
  ) {
    teams.push(props.calculation.greatestBid.team);
  }
  if (
    props.calculation.highestTrump &&
    !teams.some((t) => t.compare(props.calculation.highestTrump!.team) === 0)
  ) {
    teams.push(props.calculation.highestTrump.team);
  }
  if (
    props.calculation.lowestTrump &&
    !teams.some((t) => t.compare(props.calculation.lowestTrump!.team) === 0)
  ) {
    teams.push(props.calculation.lowestTrump.team);
  }
  if (
    props.calculation.jackiestTrump &&
    !teams.some((t) => t.compare(props.calculation.jackiestTrump!.team) === 0)
  ) {
