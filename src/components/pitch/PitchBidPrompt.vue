...
import type { PitchBidder, PitchCard, PitchCardRank, PitchCardSuit, PitchTeam } from '@/lib/pitch/classes';
...
/* store */
const { currentState, currentRound, currentTrick } = storeToRefs(usePitchStore());
/* refs */
const bid = ref(0);
...
const validBids = computed(() => {
    ...
    return [{
        ...
        bid: 5,
        enabled: currentState.value.options.isBidFiveEnabled &&
            (!bid || bid.bid < 5)
    }]
})

function returnValue(): IPitchBid<PitchCardRank, PitchCardSuit, PitchCard, PitchTeam, PitchBidder> {
    active.value = false;
        ...
        <template #default>
            <div class="container">
                <section id="pitch-bid-bid" class="content">
                    {{t(textKeys.message_bid_for, [bidder.name])}}
                    <div class="select">
                        <select v-model="bid">
                            <option v-for="o in validBids" :disabled="!o.enabled" :selected="o.bid === bid"
                                :value="o.bid">
                                {{ o.bid }}
                            </option>
                        </select>
                ...
                <section id="pitch-bid-submit" class="content level is-mobile">
                    <button class="level-item button is-success" @click="placeBid()">{{t(textKeys.label_bid_submit)}}</button>
                    <button class="level-item button" :disabled="!skippable" @click="skipBid()">{{t(textKeys.label_bid_skip)}}</button>
                </section>
            </div>
        </template>
